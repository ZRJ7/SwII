...
#PCA
pca <- read_table2("Fig4DE_Sw_merged_b1_b2_filtered_chr1.eigenvec", col_names = FALSE)
eigenval <- scan("Fig4DE_Sw_merged_b1_b2_filtered_chr1.eigenval")
pve <- data.frame(PC = 1:length(eigenval), pve = eigenval/sum(eigenval)*100)
pca <- pca[-1,-1]
names(pca)[1] <- "ind"
names(pca)[2:ncol(pca)] <- paste0("PC", 1:(ncol(pca)-1))
labs <- read.table("Fig4DE_Sw_g3_all.fam")
pca$labs<-(labs[,6])
pca<-data.frame(pca)
...
tocgff<-read.table("Fig4DE_TOC.asm.scaffold.gene.gff3") %>% 
  mutate(V1=as.integer(gsub("scaffold_","",V1)),
         gene=gsub("ID=.*;.*Name=","",V9)) %>%
  filter(V3=="gene")

#read functional annotations
gene_annos<-read.table("Fig4DE_TOC.asm.scaffold.gene.SWISSPROT.blastp.top_hit.txt")
...
Sw<-read.csv("Fig4DE_Sw_merged_b1_b2_filteredP0.1.csv",h=T)
Sw$chr<-as.integer(gsub("scaffold_","",Sw$chr))
Sw$sig<-Sw$padj<0.05

#manhattan plot
Sw.plot<-ggplot(Sw,aes(x=ps,y=(-log10(p_lrt)),colour=sig))+
  geom_rect(data=data.frame(chr=1),ymin=0,ymax=150,xmin=96e+06,xmax=246e+06,inherit.aes=FALSE,colour='#ffffff',fill='#f0f5ed',linewidth=0.05)+
  ...
  scale_colour_manual(values=c("#cccccc","#db6767"))+
  ylab("-log10(P)")
...
Sw.plot.zoomed<-ggplot(Sw[Sw$chr==1 & Sw$ps>96e+06 & Sw$ps<246e+06,],aes(x=ps/1e+06,y=(-log10(p_lrt)),colour=sig))+
  geom_rect(data=data.frame(chr=1),ymin=0,ymax=150,xmin=96,xmax=246,inherit.aes=FALSE,colour='#ffffff',fill='#f0f5ed',linewidth=0.05)+
...
#add DEGs
degs<-read.csv("Fig4DE_Sw_degs.csv",h=T)
...
dge.plot<-ggplot(degs[degs$chr %in% c(1:14),],aes(x=start,y=(log10(pvalue))))+
  ...
  geom_point(data=degs[degs$chr %in% c(1:14) & !(degs$padj<0.05 & abs(degs$log2FoldChange)>1),],
             aes(x=start,y=(log10(pvalue))),colour="#cccccc",size=0.5)+
  geom_point(data=degs[degs$chr %in% c(1:14) & (degs$padj<0.05 & abs(degs$log2FoldChange)>1),],
  ...
  scale_colour_manual(values=c("#cccccc","#db6767"))+scale_y_continuous(limits=c(-150, -1.3),expand=c(0,0))+
  ylab("-log10(P)")
