...
mes<-read.csv("FigS2 Temperature experiment.csv")
...
g.len<-ggplot(mes,aes(colour=factor(temp),group=sw.yn,x=log10(pronotum),y=log10(as.numeric(rwing_length))))+
  ...
  theme_minimal()+scale_colour_manual(name = "Temp (°C)",
                                      values=c('#4789b5', '#e6b327', '#c73838'))+
  scale_fill_manual(name = "Temp (°C)",
                    values=c('#4789b5', '#e6b327', '#c73838'))+
  geom_smooth(data=mes[!mes$sw.yn=="Sw",],
              aes(colour=factor(temp),fill=factor(temp),group=paste0(temp,sex),x=log10(pronotum),y=log10(as.numeric(rwing_length))),
              method='lm')+
  facet_grid(.~sex,scales='free')+theme(panel.border=element_rect(colour='black', fill = NA))
g.len
